...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST' && action === 'validate-code') {
      ...
      const candidates = code.length === 12
        ? [`${code.slice(0,4)}-${code.slice(4,8)}-${code.slice(8,12)}`, code]
        : [codeInput, code];

      let rows = [];

      // First try the normal SHA-256 hash lookup.
      ...
      // Fallback: compare the submitted code with the encrypted server-side
      // copy. This protects against formatting/hash mismatches while keeping
      // the plaintext code out of the database.
      if (!rows.length) {
        const activeCodes = await query(
          "SELECT id,code_ciphertext FROM registration_codes WHERE status='ACTIVE' AND expires_at>now() AND code_ciphertext IS NOT NULL"
        );

        const normalizedInput = code.replace(/[^A-Z0-9]/g, '');

        for (const item of activeCodes) {
          const storedCode = decryptSecret(item.code_ciphertext);
          if (!storedCode) continue;

          const normalizedStored = String(storedCode)
            .toUpperCase()
            .replace(/[^A-Z0-9]/g, '');
          ...
          rows = await query(
            "UPDATE registration_codes SET status='RESERVED',reserved_until=LEAST(now()+interval '30 minutes', expires_at) WHERE id=$1 AND status='ACTIVE' AND expires_at>now() RETURNING id,expires_at",
            [item.id]
          );
